...
Page({
	...
	getLineData: function (e) {
		...
		util.ajax({
			url: "data-analysis/api/sg/salesTrend",
			method: "POST",
			data: sendData,
			success: res => {
				if (res.success) {
					...
					for (let i = 0; i < res.data.length; i++) {
						lineChartData.xData.push(res.data[i].timestr.substring(6, 8));
						lineChartData.seriesData.push(res.data[i].sales);
					}
				}
				let chartSet = function () {
					if (chart) {
		...
		util.ajax({
			...
			success: res => {
				if (res.success) {
					...
					for (let i = 0; i < res.data.length; i++) {
						lineChartData1.xData.push(res.data[i].timestr.substring(4, 6));
						lineChartData1.seriesData.push((res.data[i].sales / 10000).toFixed(2));
					}
				}
				let chartSet = function () {
					if (chart1) {
